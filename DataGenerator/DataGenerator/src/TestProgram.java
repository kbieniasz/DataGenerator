...
public class TestProgram {
    ...
    public static void main(String[] args) throws ParseException, IndexOutOfBoundsException, FileNotFoundException {
        PrintWriter outWriter1 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane1.txt");
        PrintWriter outWriter2 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane2.txt");
        PrintWriter outWriter3 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane3.txt");
        PrintWriter outWriter4 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane4.txt");
        PrintWriter outWriter5 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane5.txt");
        PrintWriter outWriter6 = new PrintWriter("C:\\Users\\Admin\\Desktop\\Bazy\\Dane6.txt");
        ...
        for (int i = 0; i<1000;i++)
        {
            ...
            listCompanies.add(new Company
                    (i+1,companyName,nip,phone,email,addres,postal,city, "USA"));
        ...
        for (int i = 0; i<2000;i++)
        {
            ...
            listOfIndividualClients.add(new IndividualClient
                    (i+1001,firstName,lastName,phone,email,addres,postal,city, "USA"));
        }
        ...
        while(k < 75)   // w tym warunku możemy ustalić dowolną ilość konferencji
        {
            ...
            for(int tmp=0;tmp<=listConferences.get(k).getLenght();tmp++)
            {
                ...
                // generowanie warszatów dla poszczególnych dni
                n = rand.nextInt(4) + 2;
                ...
                for(int i = 0; i<n;i++)
                {
                    ...
                    Workshop w = new Workshop(workshopCounter+1,conferenceDayCounter+1,
        ...
        //rezerwacji dokonują zarówną klienci indywidualni jak i grupowi
        ...
        for(int i= 0; i<listConferences.size();i++)
        {
            ...
            while(kj<selectedCompany.size())
            {
                ...
                bookingCalendar.add(Calendar.DATE,((-1)*n));
                tmpStartDateDate = bookingCalendar.getTime();
                tmpStartDate = dateFormat.format(tmpStartDateDate);


                ConferenceBooking cb = new ConferenceBooking(conferenceBookingCounter+1,
                        con.getConferenceID(), selectedCompanyID, tmpStartDate);
                ...
                for(int q = 0; q<selectedConferenceDay.size();q++)
                {
                    ...
                    ConferenceDayBooking cdb  =
                            new ConferenceDayBooking(conferenceBookingDayCounter+1,
                    ...
                    for(int qwe = 0; qwe<selectedWorkshops.size();qwe++)
                    {
                        ...
                        if(listWorkshops.get(selectedWorkshopID-1).getCurrentOccupancy() + possibleAttendees <=
                                listWorkshops.get(selectedWorkshopID-1).getPeopleLimit() && possibleAttendees>0)
                        {
                            WorkshopBooking wb =
                                    new WorkshopBooking(workshopBookingCounter+1,
            ...
            while(kj<selectedIndividual.size())
            {
                ...
                ConferenceBooking cb = new ConferenceBooking(conferenceBookingCounter+1,
                        con.getConferenceID(), selectedIndividualID, tmpStartDate);
                listConferenceBooking.add(cb);
                kj++;
                conferenceBookingCounter++;
                ...
                for(int q = 0; q<selectedConferenceDay.size();q++)
                {
                    ...
                    for(int qwe = 0; qwe<selectedWorkshops.size();qwe++)
                    {
                        ...
                        if(listWorkshops.get(selectedWorkshopID-1).getCurrentOccupancy() +
                                possibleAttendees <= listWorkshops.get(selectedWorkshopID-1).getPeopleLimit()
                                && possibleAttendees > 0)
                        {
                            WorkshopBooking wb =
                                    new WorkshopBooking(workshopBookingCounter+1,
        ...
        for(int i=0;i<listConferenceBooking.size();i++)
        {
            ...
            for(int j=0; j<listPrices.size();j++)
            {
                ...
                if(dd.compareTo(d1)>=0 && dd.compareTo(d2) <= 0 &&
                        listPrices.get(j).getConferenceID() == cb.getConferenceID())
                {
            ...
            for (int icd = 0 ; icd<selectedConferenceDayBooking.size();icd++)
            {
                valueOfBooking = valueOfBooking +
                        (selectedConferenceDayBooking.get(icd).getNumberOfParticipants()- selectedConferenceDayBooking.get(icd).getNumberOfStudents())*pricePerDay +
        ...
        // wygenerowanie poszczególnych rekordów w DayReservation i WorkshopReservation
        for(int i = 0; i<listConferenceDayBooking.size();i++)
        {
            ...
            for(int qaz = 0; qaz < normal; qaz++)
            {
                ...
                DayReservation dr = new DayReservation(dayReservationCounter+1,
            ...
            for(int wri = 0; wri<selectedWorkshopBooking.size();wri++)
            {
                ...
                for(int edc = 0; edc < wb.getNumberOfParticipnats(); edc++)
                {
                    ...
                    WorkshopReservation wr =
                            new WorkshopReservation(workshopReservationCounter+1,
